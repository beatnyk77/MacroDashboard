...
export async function computeIndiaPositioning(supabase: ReturnType<typeof createClient>): Promise<IngestResult> {
  ...
  const points: DailySignalPoint[] = [...byDate.entries()].map(([date, values]) => ({ date, fii: values.IN_FII_CASH_NET ?? 0, dii: values.IN_DII_CASH_NET ?? 0, nifty: values.IN_NIFTY_RETURN ?? null, breadth: values.IN_MARKET_BREADTH ?? null, vix: values.IN_INDIA_VIX ?? null, usdInr: values.IN_USD_INR_RETURN ?? null, liquidity: values.IN_RBI_LIQUIDITY_IMPULSE ?? null, credit: values.IN_BANK_CREDIT_GROWTH_YOY ?? null }));
