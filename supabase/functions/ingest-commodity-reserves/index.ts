...
async function doIngestCommodityReserves(supabase: any, eiaApiKey: string, gieApiKey: string | undefined): Promise<IngestResult> {
    ...
    return {
        ok: true,
        counts: { upserted: reservesByCountry.length, skipped },
        meta: { steps: stepLogs },
    }
}

serveIngest('ingest-commodity-reserves', async (_req: Request): Promise<IngestResult> => {
    ...
    return doIngestCommodityReserves(supabase, eiaApiKey, gieApiKey)
