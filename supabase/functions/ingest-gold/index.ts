...
async function doIngestGold(supabase: any, avApiKey: string) {
...
Deno.serve(async (req: Request) => {
    ...
    return runIngestion(supabase, 'ingest-gold', async (ctx) => {
        const result = await runWithRetry(
            'ingest-gold',
            () => doIngestGold(ctx.supabase, avApiKey),
            { timeoutMs: 15 * 60 * 1000, maxRetries: 3 }
        );

        if (!result.ok) throw new Error(`Gold ingestion failed: ${result.error}`);
        return result.value!;
    })
})
