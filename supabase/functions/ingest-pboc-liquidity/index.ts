 ...
 */
async function doIngestPbocLiquidity(supabase: any): Promise<IngestResult> {
    ...
    await upsertObservations(supabase, metricUpserts, {
        source_ref: 'live_api:ingest-pboc-liquidity',
    ...
    return {
        ok: true,
        counts: { upserted: metricUpserts.length + 1, skipped: 0 },
        meta: { regime: regime_label, m2Growth }
    };
}

serveIngest('ingest-pboc-liquidity', async (_req: Request): Promise<IngestResult> => {
    const supabase = createClient(
        Deno.env.get('SUPABASE_URL') ?? '',
        Deno.env.get('SUPABASE_SERVICE_ROLE_KEY') ?? ''
    )
    return doIngestPbocLiquidity(supabase)
