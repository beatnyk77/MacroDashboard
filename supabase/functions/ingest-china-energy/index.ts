 ...
 */
async function doIngestChinaEnergy(supabase: any): Promise<IngestResult> {
    ...
    if (upserts.length === 0) {
        ...
        console.log(`[ChinaEnergy] Done (fallback). Upserted ${upserts.length} records.`);
...
serveIngest('ingest-china-energy', async (_req: Request): Promise<IngestResult> => {
    const supabase = createClient(
        Deno.env.get('SUPABASE_URL') ?? '',
        Deno.env.get('SUPABASE_SERVICE_ROLE_KEY') ?? ''
    )
    return doIngestChinaEnergy(supabase)
