...
async function doIngestOilGlobal(supabase: any, eiaApiKey: string): Promise<IngestResult> {
    ...
    return {
        ok: true,
        counts: { capacity_rows: capacityRowsProcessed, imports_rows: importsRowsProcessed },
        meta: { capacity: capacityRowsProcessed, imports: importsRowsProcessed }
    };
}

serveIngest('ingest-oil-global', async (_req: Request): Promise<IngestResult> => {
    const supabase = createClient(
        Deno.env.get('SUPABASE_URL') ?? '',
        Deno.env.get('SUPABASE_SERVICE_ROLE_KEY') ?? ''
    )
    const eiaApiKey = Deno.env.get('EIA_API_KEY') ?? ''
    if (!eiaApiKey) throw new Error('Missing EIA_API_KEY environment variable')
    return doIngestOilGlobal(supabase, eiaApiKey)
