...
interface ComponentScores {
  liquidity: number; rates: number; dollar: number; vol: number; metals: number;
}

interface MacroSignalResult {
  regime: MacroRegime; score: number; confidence_pct: number; key_driver: string;
  watch_item: string; component_scores: ComponentScores;
}
...
function computeLiquidityScore(regime: 'EXPANDING' | 'NEUTRAL' | 'CONTRACTING', compositeScore: number, wowPct: number): number {
...
function computeRatesScore(us10y: number, us2y: number): number {
  ...
  let base = 50;
  if (slope > 0.75) base += 20; else if (slope > 0.25) base += 10;
  else if (slope < -0.50) base -= 20; else if (slope < 0) base -= 10;
  if (us10y < 3.0) base += 12; else if (us10y < 3.75) base += 5;
  else if (us10y > 5.5) base -= 18; else if (us10y > 4.75) base -= 8;
...
function computeVolScore(sofrSpread: number): number {
  if (sofrSpread <= 3) return 82; if (sofrSpread <= 8) return 65;
  if (sofrSpread <= 15) return 48; if (sofrSpread <= 25) return 32;
  return 15;
}

function computeMetalsScore(cgZScore: number): number {
  let status = 'neutral';
  if (cgZScore > 1) status = 'safe'; else if (cgZScore < -1.5) status = 'danger';
...
function computeMacroSignal(inputs: {
  liquidityRegime: 'EXPANDING' | 'NEUTRAL' | 'CONTRACTING'; liquidityCompositeScore: number;
  liquidityWowPct: number; us10yYield: number; us2yYield: number; dxyZScore: number;
  sofrEffrSpreadBps: number; copperGoldZScore: number;
}): MacroSignalResult {
  ...
  const score = clamp(liq * WEIGHTS.liquidity + rates * WEIGHTS.rates + dollar * WEIGHTS.dollar + vol * WEIGHTS.vol + metals * WEIGHTS.metals, 0, 100);
  ...
  const DRIVER_LABELS: Record<string, string> = { liquidity: 'Global Liquidity Conditions', rates: 'Yield Curve & Rate Levels', dollar: 'US Dollar Strength (DXY)', vol: 'Funding Market Stress', metals: 'Copper/Gold Growth Proxy' };
  const WATCH_LABELS: Record<string, string> = { liquidity: 'CB balance sheet trajectory', rates: 'Next Treasury auction BTC + 10Y breakeven', dollar: 'DXY pivot vs EM FX moves', vol: 'SOFR-EFFR spread & repo market', metals: 'Copper/Gold ratio vs PMI trend' };
  const WEIGHTS_MAP: Record<string, number> = { liquidity: 0.30, rates: 0.25, dollar: 0.20, vol: 0.15, metals: 0.10 };
  const sorted = Object.entries(component_scores).sort((a, b) => Math.abs(b[1] - 50) * (WEIGHTS_MAP[b[0]] ?? 0) - Math.abs(a[1] - 50) * (WEIGHTS_MAP[a[0]] ?? 0));
...
async function doIngest(supabase: ReturnType<typeof createClient>): Promise<IngestResult> {
  ...
  const metricsNeeded = ['US_10Y_YIELD', 'US_2Y_YIELD', 'DXY_INDEX', 'SOFR_EFFR_SPREAD_BPS', 'COPPER_GOLD_RATIO', 'GOLD_COMEX_USD', 'US_10Y_TIPS_YIELD', 'CB_GOLD_NET'];
  ...
  const getVal = (id: string) => Number(metrics.find((m: any) => m.metric_id === id)?.value ?? 0);
  const getZ = (id: string) => Number(metrics.find((m: any) => m.metric_id === id)?.z_score ?? 0);
  ...
  const { error: sigError } = await supabase.from('daily_signal').upsert({
    signal_date: today, regime: signal.regime, score: signal.score,
    confidence_pct: signal.confidence_pct, score_delta, regime_changed,
    key_driver: signal.key_driver, watch_item: signal.watch_item,
    component_scores: signal.component_scores, computed_at: new Date().toISOString(),
  }, { onConflict: 'signal_date' });
  if (sigError) throw sigError;

  const { error: briefError } = await supabase.from('macro_brief').upsert({
    signal_date: today, regime_line, driver_line, watch_line, context_line,
  ...
  if (goldZ > 1.0 && realYield > 1.5) contradictions.push({ contradiction_key: 'gold_vs_real_yields', title: 'Gold ↑ while Real Yields ↑', interpretation: 'Gold is rising despite positive real returns on Treasuries — geopolitical risk premium or de-dollarization demand beyond inflation hedging.', severity: goldZ > 2 ? 'EXTREME' : 'NOTABLE', metric_a: 'GOLD_COMEX_USD', metric_b: 'US_10Y_TIPS_YIELD' });
  if (cgZ < -1.0 && us10y < 3.5) contradictions.push({ contradiction_key: 'copper_vs_easy_rates', title: 'Copper/Gold ↓ despite Easy Rates', interpretation: 'Growth proxy contracting even as rates remain accommodative — demand destruction that policy may not cure.', severity: cgZ < -2 ? 'EXTREME' : 'NOTABLE', metric_a: 'COPPER_GOLD_RATIO', metric_b: 'US_10Y_YIELD' });
  if (slope < -0.25 && liqRegime === 'EXPANDING') contradictions.push({ contradiction_key: 'inverted_curve_expanding_liquidity', title: 'Inverted Curve + CB Easing', interpretation: 'Bond market pricing recession while CBs expand balance sheets.', severity: slope < -0.75 ? 'EXTREME' : 'NOTABLE', metric_a: 'US_10Y_YIELD', metric_b: 'GLOBAL_CB_BALANCE_SHEET' });
  if (dxyZ > 1.5 && cbGoldYoY > 10) contradictions.push({ contradiction_key: 'strong_dollar_vs_cb_gold', title: 'Strong DXY + CB Gold Accumulation', interpretation: 'Dollar strength coexists with accelerating sovereign gold accumulation — EM central banks hedging at scale.', severity: dxyZ > 2.5 ? 'EXTREME' : 'NOTABLE', metric_a: 'DXY_INDEX', metric_b: 'CB_GOLD_NET' });

  const topContradictions = contradictions.sort((a, b) => (a.severity === 'EXTREME' ? -1 : 1) - (b.severity === 'EXTREME' ? -1 : 1)).slice(0, 2);
  ...
  if (topContradictions.length > 0) {
    const { error: insertContrError } = await supabase.from('macro_contradictions').insert(topContradictions.map(c => ({ ...c, signal_date: today })));
  ...
  const thresholds: Record<string, { pct: number; abs: number | null }> = { yield: { pct: 3, abs: 0.10 }, liquidity: { pct: 1.5, abs: 100 }, ratio: { pct: 2, abs: null }, default: { pct: 5, abs: null } };
  const changes: any[] = [];

  for (const m of watchMetrics) {
    const curr = Number(metrics.find((x: any) => x.metric_id === m.id)?.value ?? null);
    if (!curr) continue;
    const { data: prevData, error: prevError } = await supabase.from('metric_observations').select('value').eq('metric_id', m.id).lt('as_of_date', today).order('as_of_date', { ascending: false }).limit(1).maybeSingle();
    ...
    changes.push({ signal_date: today, metric_id: m.id, metric_label: m.label, prev_value, curr_value: curr, abs_delta, pct_delta, significance, direction, interpretation: `${m.label} ${direction === 'UP' ? 'rose' : 'fell'} ${Math.abs(pct_delta).toFixed(2)}% overnight.` });
  }
  ...
  return { ok: true, counts: { upserted: 1 + topContradictions.length + changes.length, skipped: 0 }, meta: { date: today, regime: signal.regime, score: signal.score, contradictions: topContradictions.length, changes: changes.length } };
}
