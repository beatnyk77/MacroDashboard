 ...
 */
async function doIngestChinaDefaults(supabase: any): Promise<IngestResult> {
    ...
    const { count } = await upsertObservations(supabase, [observation], {
        source_ref: 'live_api:ingest-china-defaults',
    ...
    return {
        ...
        meta: {
            as_of_date: asOfDate,
            calculated_rate: Number(calculatedRate),
...
serveIngest('ingest-china-defaults', async (_req: Request): Promise<IngestResult> => {
    const supabase = createClient(
        Deno.env.get('SUPABASE_URL') ?? '',
        Deno.env.get('SUPABASE_SERVICE_ROLE_KEY') ?? ''
    )
    return doIngestChinaDefaults(supabase)
