...
async function doIngest(supabase: ReturnType<typeof createClient>): Promise<IngestResult> {
    ...
    for (const metric of pulseMetrics) {
        ...
        if (metricError) {
            console.error(`Error upserting metric for ${metric.iso}:`, metricError)
            skipped++
        }
    }
    ...
    return {
        ok: true,
        counts: { upserted: pulseMetrics.length - skipped + 1, skipped },
        meta: { snapshot_date: snapshotDate, pulse_metrics: pulseMetrics.length }
    };
}

serveIngest('ingest-africa-macro', async (_req: Request): Promise<IngestResult> => {
