...
async function doIngest(supabase: ReturnType<typeof createClient>, req: Request): Promise<IngestResult> {
    ...
    const flowCode = urlParams.get('flowCode') || 'X';
    const partnerCode = urlParams.get('partnerCode') || 'all';
    ...
    if (tradeData.length > 0) {
        ...
        upserted = bilateralData.length;
        console.log(`Successfully upserted ${bilateralData.length} records.`);
    }

    return {
        ok: true,
        counts: { upserted, skipped: tradeData.length - upserted },
        meta: { category, hs_code: hsCode, flow_code: flowCode, total_records: tradeData.length },
    };
}

serveIngest('ingest-un-comtrade', async (req: Request): Promise<IngestResult> => {
