...
async function doIngest(supabase: ReturnType<typeof createClient>): Promise<IngestResult> {
    ...
    return {
        ok: true,
        counts: { upserted: filteredArticles.length, skipped: 0 },
        meta: { total_fetched: articles.length, total_filtered: filteredArticles.length }
    };
}

serveIngest('ingest-macro-news-headlines', async (_req: Request): Promise<IngestResult> => {
