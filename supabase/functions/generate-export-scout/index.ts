...
Deno.serve(async (req: Request): Promise<Response> => {
  ...
  // Only pull DG companion data and use static fallback when the queried code is 8512 family
  ...
  try {
    ...
    dgDemand = isDualProduct ? dedup((dgDemandRes.data ?? []) as DemandRow[]) : [];
    ltIndia  = dedup((ltIndiaRes.data  ?? []) as SupplierRow[]);
    dgIndia  = isDualProduct ? dedup((dgIndiaRes.data  ?? []) as SupplierRow[]) : [];
  } catch (err) {
  ...
  // STATIC_FALLBACK is only meaningful for HS 8512 (Light Tower) context
  const useStaticFallback = isDualProduct;
  ...
  const allIso3 = new Set([
    ...
    ...(useStaticFallback ? Object.keys(STATIC_FALLBACK) : []),
  ]);
  ...
  for (const iso3 of allIso3) {
    ...
    const fallback = useStaticFallback ? STATIC_FALLBACK[iso3] : undefined;
    ...
    const lead = isDualProduct
      ? (fallback?.lead ?? calcLead(lt_m, dg_m, lt_i, dg_i))
      : (lt_i >= 15 ? "India Stronghold" : lt_i >= 5 ? "India Active" : lt_i >= 1 ? "Emerging" : "Opportunity");
    const countryLabel = COUNTRY_DISPLAY[iso3] ??
      (ltRow?.reporter_name ?? dgRow?.reporter_name ?? iso3);
    const gtm = GTM_TEXT[iso3] ?? (isDualProduct
      ? `India share LT ${lt_i}% / DG ${dg_i}%. Find local importer.`
      : `India share ${lt_i}%. Find local importer/distributor for HS ${hsn}.`);
  ...
  const phase1Rows = phase1Markets.map(m => {
    ...
    const leadCls = (!isDualProduct || m.lead === "Light Tower" || m.lead === "Both") ? "tier-p1" : "tier-p2";
    return `<tr>
...
function buildHtml(p: {
  ...
  const { pageTitle, headerSubtitle, tamDisplay, countryCount, indiaShareGlobal,
          phase1Display, phase1Rows, marketsJson, hsn, hsnDescription, isDualProduct } = p;
...
<div class="header">
  ...
  <div class="header-right">
    ...
    <div>${esc(hsnDescription)} (${countryCount} countries)</div>
  ...
  <div class="section active" id="sec-dashboard">
    <div class="stat-row">
      ...
      <div class="stat-box" style="border-top-color:#888">
        <div class="val">${isDualProduct ? 2 : 1}</div>
        <div class="lbl">HS Code${isDualProduct ? "s" : ""} to Track</div>
      </div>
    </div>
    ...
    <div class="card">
      ...
      <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:14px;">
        <div style="padding:14px;background:var(--paper);border-radius:5px;">
          ...
          <div style="font-size:12px;color:var(--muted);">Run Volza searches for all 5 Phase 1 countries. Build list of 20+ importers per country who already import HS ${hsn} from India.</div>
        </div>
        <div style="padding:14px;background:var(--paper);border-radius:5px;">
  ...
  <div class="section" id="sec-markets">
    ...
    <div class="card" style="padding:0;overflow:hidden;">
      <table class="market-table" id="market-table">
        <thead><tr>
          <th>#</th><th>Country</th>${isDualProduct
            ? "<th>LT Market</th><th>DG Market</th><th>Total</th><th>India LT%</th><th>India DG%</th>"
            : "<th>Market Size</th><th>India Share %</th>"}<th>Strategic Tier</th><th>Opportunity</th><th>GTM Action</th>
        </tr></thead>
  ...
  <div class="modal">
    ...
    <div class="form-row">
      ...
      <div class="form-group">
        ...
        <select id="f-product">
          <option>${esc(hsnDescription)} (HS ${hsn})</option>
          <option>Other</option>
        </select>
      </div>
    </div>
...
const IS_DUAL_PRODUCT = ${isDualProduct};
...
function renderMarkets(data) {
  ...
  tbody.innerHTML = data.map((m,i)=>{
    const dataCols = IS_DUAL_PRODUCT
