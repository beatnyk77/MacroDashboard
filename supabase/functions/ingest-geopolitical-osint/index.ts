...
async function doIngest(supabase: ReturnType<typeof createClient>, openSkyUser?: string, openSkyPass?: string): Promise<IngestResult> {
    const results = [];
    ...
    for (const box of boxes) {
        try {
            ...
            const aircraftRes = await fetchWithTimeout(openSkyUrl, { timeout: 40000, headers });
...
serveIngest('ingest-geopolitical-osint', async (_req: Request): Promise<IngestResult> => {
    ...
    return doIngest(supabase, openSkyUser, openSkyPass)
