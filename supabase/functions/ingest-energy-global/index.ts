...
async function doIngestEnergyGlobal(supabase: any): Promise<IngestResult> {
            ...
            for (const entity of entities) {
                ...
                if (res.ok) {
                    ...
                    if (latestDate) {
                        ...
                        observations.push(
                            { metric_id: `${prefix}_POWER_COAL_PCT`, value: coal, as_of_date: latestDate },
                            { metric_id: `${prefix}_POWER_RENEWABLE_PCT`, value: renewables, as_of_date: latestDate },
                            { metric_id: `${prefix}_POWER_OTHER_PCT`, value: other, as_of_date: latestDate }
                        );
                    }
                }
            }
    ...
    const allObservations = [...observations, ...gasObservations];
    let upserted = 0;
    if (allObservations.length > 0) {
        const { count } = await upsertObservations(supabase, allObservations, {
            source_ref: 'live_api:ingest-energy-global',
    ...
    return {
        ...
        meta: { ember_obs: observations.length, gas_obs: gasObservations.length }
    };
}

serveIngest('ingest-energy-global', async (_req: Request): Promise<IngestResult> => {
    const supabase = createClient(
        Deno.env.get('SUPABASE_URL') ?? '',
        Deno.env.get('SUPABASE_SERVICE_ROLE_KEY') ?? ''
    )
    return doIngestEnergyGlobal(supabase)
