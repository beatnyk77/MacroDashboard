-- Extend vw_country_terminal to support top 40 countries with GMD supplement data
-- Adds 12 new GMD macro fundamental columns and expands country filter from 25 → 40
...
COMMENT ON VIEW vw_country_terminal IS
  'Country macro terminal for top 40 economies. Combines FRED real-time metrics (policy rates, yield curves) with Global-Macro-Database annual fundamentals (GDP, debt, trade). NULLs indicate metric not available for that country.';
