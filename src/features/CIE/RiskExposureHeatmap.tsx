        ...
        <div className="space-y-6">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 px-8 py-6 rounded-[2rem] bg-white/[0.02] border border-white/5 backdrop-blur-3xl">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <h2 className="text-xl font-black uppercase tracking-heading text-white leading-tight">Macro Risk Gameboard</h2>
                        <p className="text-xs text-muted-foreground/60 font-black uppercase tracking-uppercase mt-1">Sectoral exposure to systemic macro shocks</p>
                    </div>
                </div>
                ...
                <div className="flex bg-black/40 p-1.5 rounded-2xl border border-white/12 backdrop-blur-md">
                    <button
                        ...
                        className={`px-6 py-2 rounded-xl text-xs font-black uppercase tracking-uppercase transition-all ${viewMode === 'sector' ? 'bg-white/10 text-white shadow-lg' : 'text-white/30 hover:text-white/60'}`}
                    >
                        Sector Averages
                    </button>
                    <button
                        onClick={() => setViewMode('company')}
                        className={`px-6 py-2 rounded-xl text-xs font-black uppercase tracking-uppercase transition-all opacity-30 cursor-not-allowed`}
                        disabled
                    >
                        Drill-down
                    </button>
                ...
                <table className="w-full text-left border-collapse">
                    <thead>
                        <tr className="border-b border-white/5 bg-white/[0.01]">
                            <th className="px-8 py-6 text-xs font-black uppercase tracking-uppercase text-white/30">Sector Structure</th>
                            <th className="px-6 py-6 text-xs font-black uppercase tracking-uppercase text-white/30 text-center">Oil Sensitivity</th>
                            <th className="px-6 py-6 text-xs font-black uppercase tracking-uppercase text-white/30 text-center">Fiscal Risk</th>
                            <th className="px-6 py-6 text-xs font-black uppercase tracking-uppercase text-white/30 text-center">Formalization</th>
                            <th className="px-6 py-6 text-xs font-black uppercase tracking-uppercase text-white/30 text-center">Liquidity</th>
                            <th className="px-6 py-6 text-xs font-black uppercase tracking-uppercase text-white/30 text-center">CDS Spread</th>
                            <th className="px-8 py-6 text-xs font-black uppercase tracking-uppercase text-white/30 text-right">Risk Pulse</th>
                        </tr>
                    </thead>
                                ...
                                <tr key={row.sector} className="group hover:bg-blue-500/[0.02] transition-all cursor-pointer" onClick={handleFilterClick}>
                                    <td className="px-8 py-6">
                                        <div className="flex flex-col">
                                            <span className="text-sm font-black text-white group-hover:text-blue-400 transition-colors tracking-heading flex items-center gap-2">
                                                {row.sector}
                                                <ArrowUpRight size={14} className="opacity-0 group-hover:opacity-100 transition-opacity" />
                                            </span>
                                            <div className="text-xs font-black text-white/30 uppercase tracking-uppercase mt-1">{row.companyCount} Equities</div>
                                        </div>
                                    </td>

                                    <td className="px-6 py-6 text-center">
                                        <div className={`inline-flex items-center justify-center px-4 py-2 rounded-xl border text-xs font-black italic tracking-heading w-16 ${getColorScore(row.oilSensitivity)}`}>
                                            {row.oilSensitivity}
                                        </div>
                                    </td>
                                    <td className="px-6 py-6 text-center">
                                        <div className={`inline-flex items-center justify-center px-4 py-2 rounded-xl border text-xs font-black italic tracking-heading w-16 ${getColorScore(row.stateFiscalRisk)}`}>
                                            {row.stateFiscalRisk}
                                        </div>
                                    </td>
                                    <td className="px-6 py-6 text-center">
                                        <div className={`inline-flex items-center justify-center px-4 py-2 rounded-xl border text-xs font-black italic tracking-heading w-16 ${getColorScore(row.formalizationRisk)}`}>
                                            {row.formalizationRisk}
                                        </div>
                                    </td>
                                    <td className="px-6 py-6 text-center" onClick={(e) => { e.stopPropagation(); setSelectedSectorPanel(row); }}>
                                        <div className={`inline-flex items-center justify-center px-4 py-2 rounded-xl border text-xs font-black italic tracking-heading w-16 shadow-[0_0_15px_rgba(0,0,0,0.2)] hover:scale-105 transition-transform ${getColorScore(row.liquidityRisk)}`}>
                                            {row.liquidityRisk}
                                        </div>
                                    </td>
                                    <td className="px-6 py-6 text-center" onClick={(e) => { e.stopPropagation(); setSelectedCDSPanel(row); }}>
                                        <div className={`inline-flex items-center justify-center px-4 py-2 rounded-xl border text-xs font-black italic tracking-heading w-16 shadow-[0_0_15px_rgba(0,0,0,0.2)] hover:scale-105 transition-transform ${getColorScore(row.cdsRisk, false, true)}`}>
                                            {row.cdsRisk}
                                        </div>
                                    </td>
                                    <td className="px-8 py-6 text-right">
                                        <div className={`inline-flex items-center justify-center px-4 py-2 rounded-xl border text-xs font-black italic tracking-heading w-16 ${getColorScore(row.macroScore, true)}`}>
                                            {row.macroScore}
                                        </div>
                                    </td>
                                </tr>
            ...
            <div className="p-8 rounded-[2rem] bg-white/[0.01] border border-dashed border-white/12 flex items-start gap-6 text-white/40">
                ...
                <div>
                    <h6 className="text-xs font-black uppercase tracking-uppercase text-white/60 mb-2">Protocol Brief: Risk Gameboard Intelligence</h6>
                        ...
                        >
                            <div className="flex justify-between items-center mb-12 border-b border-white/5 pb-8">
                                <div className="flex items-center gap-4">
                                    ...
                                    <div>
                                        <h3 className="text-2xl font-black text-white leading-none tracking-heading">{selectedSectorPanel.sector}</h3>
                            ...
                            <div className="space-y-10">
                                <div className="grid grid-cols-2 gap-6">
                                    <div className="bg-white/[0.02] p-6 rounded-[2rem] border border-white/5">
                                        <div className="text-xs font-black uppercase tracking-uppercase text-white/30 mb-2">Transmission</div>
                                        <div className={`text-4xl font-black italic tracking-heading ${selectedSectorPanel.liquidityRisk > 66 ? 'text-rose-400' : selectedSectorPanel.liquidityRisk > 33 ? 'text-yellow-400' : 'text-emerald-400'}`}>
                                            {selectedSectorPanel.liquidityRisk}<span className="text-sm ml-1 opacity-20">/100</span>
                                        </div>
                                    </div>
                                    <div className="bg-white/[0.02] p-6 rounded-[2rem] border border-white/5">
                                        <div className="text-xs font-black uppercase tracking-uppercase text-white/30 mb-2">Constituents</div>
                                        <div className="text-4xl font-black text-white italic tracking-heading">{selectedSectorPanel.companyCount}</div>
                                    </div>
                                </div>

                                <div className="bg-blue-500/5 border border-blue-500/20 p-8 rounded-[2rem]">
                                    <h4 className="text-xs font-black uppercase tracking-uppercase text-blue-400 mb-4">Fundamental Driver</h4>
                                    ...
                                    <button
                                        ...
                                        className="inline-flex items-center gap-2 text-xs font-black uppercase tracking-uppercase text-blue-400 hover:text-blue-300 transition-colors group/btn"
                                    >
                                ...
                                <div>
                                    <h4 className="text-xs font-black uppercase tracking-uppercase text-white/30 mb-6 border-b border-white/5 pb-4">High Exposure Constituents</h4>
                                    <div className="space-y-3">
                                        {selectedSectorPanel.topExposed.map((comp: any) => (
                                            <div key={comp.ticker} role="button" tabIndex={0} aria-label={`View details for ${comp.ticker.replace('.NS', '')}`} onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); navigate(`/india-equities/${comp.ticker.replace('.NS', '')}`); } }} className="flex justify-between items-center p-5 rounded-2xl bg-white/[0.02] border border-white/5 hover:bg-white/[0.05] transition-all group/comp cursor-pointer focus-visible:ring-2 focus-visible:ring-blue-500 focus-visible:ring-offset-2 outline-none" onClick={() => navigate(`/india-equities/${comp.ticker.replace('.NS', '')}`)}>
                                                <div>
                                                    <div className="text-sm font-black text-white group-hover/comp:text-blue-400 transition-colors tracking-heading">{comp.ticker.replace('.NS', '')}</div>
                                                    <div className="text-xs font-medium text-white/30 truncate max-w-[200px] mt-1">{comp.name}</div>
                                                </div>
                                                <div className="flex items-center gap-4">
                                                    <div className="text-xs font-black text-rose-400 bg-rose-500/10 px-3 py-1.5 rounded-full border border-rose-500/20">
                        ...
                        >
                            <div className="flex justify-between items-center mb-12 border-b border-white/5 pb-8">
                                <div className="flex items-center gap-4">
                                    ...
                                    <div>
                                        <h3 className="text-2xl font-black text-white leading-none tracking-heading">{selectedCDSPanel.sector}</h3>
                            ...
                            <div className="space-y-10">
                                <div className="grid grid-cols-2 gap-6">
                                    <div className="bg-white/[0.02] p-6 rounded-[2rem] border border-white/5">
                                        <div className="text-xs font-black uppercase tracking-uppercase text-white/30 mb-2">5Y CDS Spread</div>
                                        <div className={`text-4xl font-black italic tracking-heading ${selectedCDSPanel.cdsRisk > 150 ? 'text-rose-400' : selectedCDSPanel.cdsRisk > 80 ? 'text-yellow-400' : 'text-emerald-400'}`}>
                                            {selectedCDSPanel.cdsRisk}<span className="text-sm ml-1 opacity-20">bps</span>
                                        </div>
                                    </div>
                                    <div className="bg-white/[0.02] p-6 rounded-[2rem] border border-white/5">
                                        <div className="text-xs font-black uppercase tracking-uppercase text-white/30 mb-2">30D Movement</div>
                                        <div className={`text-4xl font-black italic tracking-heading ${selectedCDSPanel.cdsMonthlyChange > 0 ? 'text-rose-400' : 'text-emerald-400'}`}>
                                ...
                                <div className="bg-purple-500/5 border border-purple-500/20 p-8 rounded-[2rem]">
                                    <h4 className="text-xs font-black uppercase tracking-uppercase text-purple-400 mb-4">Market Sentiment</h4>
                                ...
                                <div>
                                    <h4 className="text-xs font-black uppercase tracking-uppercase text-white/30 mb-6 border-b border-white/5 pb-4">Most Sensitive Constituents</h4>
                                    <div className="space-y-3">
                                ...
                                <div className="p-8 bg-white/[0.01] rounded-[2rem] border border-white/5">
                                    <h4 className="text-xs font-black uppercase tracking-uppercase text-white/30 mb-4">Linked Macro Stressors</h4>
                                    <div className="flex flex-wrap gap-2">
                                        {['Liquidity Transmission', 'Fiscal Staleness', 'FX Volatility'].map(tag => (
                                            <span key={tag} className="px-3 py-1.5 bg-white/5 rounded-full text-xs font-black uppercase tracking-uppercase text-white/40 border border-white/12">{tag}</span>
                                        ))}
                                    </div>
                                </div>
                            </div>
