    ...
    return (
        <div className="space-y-6">
            ...
            <div className="flex flex-col md:flex-row gap-6 items-center justify-between p-8 rounded-[2rem] bg-white/[0.02] border border-white/5 backdrop-blur-3xl">
                ...
                <div className="flex gap-4 w-full md:w-auto">
                    <select
                        ...
                        className="bg-black/40 border border-white/12 rounded-2xl py-3 px-6 text-xs font-black uppercase tracking-uppercase text-white/40 focus:outline-none focus:border-blue-500/50 appearance-none hover:text-white/60 transition-colors cursor-pointer"
                    >
            ...
            <div className="overflow-x-auto rounded-[2.5rem] border border-white/5 bg-black/40 backdrop-blur-3xl">
                <table className="w-full text-left border-collapse">
                    <thead>
                        <tr className="border-b border-white/5 bg-white/[0.01]">
                            <th className="px-8 py-6 text-xs font-black uppercase tracking-uppercase text-white/30">Company</th>
                            <th className="px-6 py-6 text-xs font-black uppercase tracking-uppercase text-white/30">Sector</th>
                            <th className="px-6 py-6 text-xs font-black uppercase tracking-uppercase text-white/30 text-right">Size (Cr)</th>
                            <th className="px-6 py-6 text-xs font-black uppercase tracking-uppercase text-white/30 text-right">Price Band</th>
                            <th className="px-8 py-6 text-xs font-black uppercase tracking-uppercase text-white/30 text-right">Dates</th>
                            <th className="px-6 py-6 text-xs font-black uppercase tracking-uppercase text-white/30 text-center">Macro Risk</th>
                            <th className="px-6 py-6 text-xs font-black uppercase tracking-uppercase text-white/30 text-center">Status</th>
                            ...
                            {filteredIPOs.map((ipo, i) => (
                                ...
                                >
                                    <td className="px-8 py-6">
                                        <div className="font-black text-white text-sm tracking-heading group-hover:text-blue-400 transition-colors">{ipo.company_name}</div>
                                        <div className="text-xs font-black uppercase tracking-uppercase text-white/20 mt-1">{ipo.exchange}</div>
                                    </td>
                                    <td className="px-6 py-6">
                                        <span className="text-xs font-black uppercase tracking-uppercase text-blue-400/80 bg-blue-400/5 px-3 py-1.5 rounded-full border border-blue-400/10">
                                            {ipo.sector}
                                        </span>
                                    </td>
                                    ...
                                    <td className="px-6 py-6 text-right">
                                        <div className="text-xs font-black text-white/80 italic tracking-heading">
                                            ₹{ipo.price_band_min} - {ipo.price_band_max}
                                        </div>
                                    </td>
                                    <td className="px-8 py-6 text-right">
                                        <div className="inline-flex items-center gap-2 text-xs font-black tracking-uppercase text-white/60 uppercase italic">
                                            <Calendar size={12} className="text-white/20" />
                                            {ipo.open_date ? format(new Date(ipo.open_date), 'dd MMM') : '--'} - {ipo.close_date ? format(new Date(ipo.close_date), 'dd MMM') : '--'}
                                        </div>
                                    </td>
                                    <td className="px-6 py-6 text-center">
                                        <div className={`inline-flex items-center gap-2 px-4 py-2 rounded-xl text-xs font-black uppercase tracking-uppercase border ${ipo.macro_risk_score < 40 ? 'bg-emerald-500/10 border-emerald-500/20 text-emerald-400' :
                                    ...
                                    <td className="px-6 py-6 text-center">
                                        <span className={`text-xs font-black uppercase tracking-uppercase ${ipo.status === 'Open' ? 'text-emerald-400 animate-pulse' :
                ...
                {filteredIPOs.length === 0 && (
                    <div className="py-32 text-center">
                        ...
                        <h3 className="text-white/30 font-black uppercase tracking-uppercase text-xs">No matching IPO candidates detected</h3>
                    </div>
                )}
            </div>
            ...
            <div className="p-8 rounded-[2.5rem] bg-orange-500/5 border border-orange-500/10 flex items-start gap-6 backdrop-blur-sm group">
                ...
                <div>
                    <h4 className="text-xs font-black uppercase tracking-uppercase text-orange-400 mb-2">Protocol Overlay: Macro Risk Intelligence</h4>
