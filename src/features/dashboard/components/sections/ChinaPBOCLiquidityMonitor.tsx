...
const REGIME_STYLES: Record<string, { bg: string; border: string; text: string; glow: string }> = {
    Easing:    { bg: 'bg-emerald-500/10', border: 'border-emerald-500/30', text: 'text-emerald-700 dark:text-emerald-400', glow: 'shadow-emerald-500/20' },
    Neutral:   { bg: 'bg-amber-500/10',   border: 'border-amber-500/30',   text: 'text-amber-700 dark:text-amber-400',   glow: 'shadow-amber-500/20' },
    Tightening:{ bg: 'bg-rose-500/10',    border: 'border-rose-500/30',    text: 'text-rose-700 dark:text-rose-400',    glow: 'shadow-rose-500/20' },
};
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-border pb-6">
                ...
                {latest && (
                    <div className={cn(
                        ...
                        <span className={cn('text-sm font-black uppercase tracking-uppercase', regimeStyle.text)}>
                            {latest.regime_label}
                        </span>
                        <span className="text-muted-foreground text-xs uppercase tracking-uppercase ml-2">PBOC Stance</span>
                    </div>
                )}
            </div>
            ...
            {isLoading ? (
                ...
                <>
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                        {[
                            { label: '1Y MLF Rate', value: latest?.mlf_rate, unit: '%', color: 'text-red-600 dark:text-red-400', desc: 'Medium-term Lending Facility' },
                            { label: '7d Reverse Repo', value: latest?.reverse_repo_7d, unit: '%', color: 'text-orange-600 dark:text-orange-400', desc: 'Short-term policy anchor' },
                            { label: 'M2 Growth YoY', value: latest?.m2_growth_yoy, unit: '%', color: 'text-amber-600 dark:text-amber-400', desc: 'Broad money supply growth' },
                            { label: 'PBOC vs Fed Gap', value: latest?.pboc_vs_fed_gap, unit: '%', color: latest?.pboc_vs_fed_gap && latest.pboc_vs_fed_gap < 0 ? 'text-rose-600 dark:text-rose-400' : 'text-emerald-700 dark:text-emerald-400', desc: 'MLF minus Fed Funds Rate' },
                    ...
                    {latest && (
                        <div className="p-4 rounded-2xl bg-card border border-border shadow-sm">
                            <div className="flex justify-between items-center mb-2">
                                <span className="text-xs font-black text-foreground uppercase tracking-uppercase">Net Liquidity Signal (M2 - Nominal GDP proxy)</span>
                                <span className={cn('text-sm font-black tabular-nums', latest.net_liquidity_signal >= 0 ? 'text-emerald-700 dark:text-emerald-400' : 'text-rose-700 dark:text-rose-400')}>
