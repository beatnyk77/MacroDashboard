        ...
        <div className="relative flex items-center justify-center">
            <svg viewBox="0 0 120 70" className="w-32 h-20">
                ...
                <line
                    ...
                    stroke="currentColor" className="text-foreground" strokeWidth="2" strokeLinecap="round"
                />
                <circle cx="60" cy="60" r="3" fill="currentColor" className="text-foreground" />
                <text x="10" y="68" fill="currentColor" className="text-muted-foreground" fontSize="7">44</text>
                <text x="53" y="15" fill="currentColor" className="text-muted-foreground" fontSize="7">50</text>
                <text x="103" y="68" fill="currentColor" className="text-muted-foreground" fontSize="7">56</text>
            </svg>
        </div>
    );
};
        ...
        <div className="bg-popover border border-border text-popover-foreground rounded-xl p-3 text-xs shadow-xl">
            ...
            {payload.map((p: any) => (
                <div key={p.name} className="flex justify-between gap-4">
                    ...
                    <span className="text-foreground font-black">{p.value?.toFixed(1)}</span>
                </div>
            ))}
            <div className="mt-1 pt-1 border-t border-border">
                <span className="text-muted-foreground">{(payload[0]?.value ?? 0) >= 50 ? '▲ Expansion' : '▼ Contraction'}</span>
            </div>
        </div>
    );
};
...
export const ChinaRealEconomyPanel: React.FC = () => {
    ...
    const pmiStatus = (v: number) => v >= 50 ? { label: 'Expanding', cls: 'text-emerald-700 dark:text-emerald-400' } : { label: 'Contracting', cls: 'text-rose-700 dark:text-rose-400' };
            ...
            {isLoading ? (
                ...
                <>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        {[
                            { label: 'NBS PMI', value: latestNBS, sub: 'Official Manufacturing PMI', color: 'text-red-600 dark:text-red-400' },
                            { label: 'Caixin PMI', value: latestCaixin, sub: 'Private sector / SME focus', color: 'text-amber-600 dark:text-amber-400' },
                            ...
                            return (
                                <div key={label} className="p-5 rounded-2xl bg-card border border-border shadow-sm flex flex-col items-center text-center gap-2">
                                    ...
                                    <p className="text-xs font-black text-foreground uppercase tracking-uppercase">{label}</p>
                                    <p className="text-xs text-muted-foreground">{sub}</p>
                                    <div className={cn('w-full h-0.5 rounded-full mt-1', value >= 50 ? 'bg-emerald-500/30' : 'bg-rose-500/30')} />
                                    <p className="text-xs text-muted-foreground/80">50.0 expansion threshold</p>
                                </div>
                            );
                        })}
                    </div>
                    ...
                    <div className="p-5 rounded-2xl bg-card border border-border shadow-sm">
                        <p className="text-xs font-black text-foreground uppercase tracking-uppercase mb-4">PMI — NBS vs Caixin (12 Months)</p>
                        <ResponsiveContainer width="100%" height={160}>
                            <LineChart data={pmiData}>
                                ...
                                <YAxis domain={[46, 54]} tick={{ fill: 'currentColor', fontSize: 9 }} tickLine={false} axisLine={false} className="text-muted-foreground" />
                                <Tooltip content={<CustomTooltip />} />
                                <ReferenceLine y={50} stroke="currentColor" className="text-border" strokeDasharray="4 4" label={{ value: '50', fill: 'currentColor', fontSize: 9, className: 'text-muted-foreground' }} />
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                        {[
                            { label: 'Industrial Prod.', data: latestIP, unit: '%', color: 'text-orange-600 dark:text-orange-400' },
                            { label: 'Retail Sales', data: latestRetail, unit: '%', color: 'text-amber-600 dark:text-amber-400' },
                            { label: 'Fixed Asset Inv.', data: latestFAI, unit: '%', color: 'text-amber-700 dark:text-yellow-400' },
                            { label: 'Corp. Distress', data: latestDistress, unit: 'pts', color: (latestDistress?.value ?? 0) < -1.5 ? 'text-rose-600 dark:text-rose-400' : 'text-amber-600 dark:text-amber-400', desc: 'CPI-PPI spread: negative = margin compression' },
                            ...
                            <div key={label} className="p-4 rounded-2xl bg-card border border-border shadow-sm hover:border-border/80 transition-all">
                                <p className="text-xs font-black text-muted-foreground uppercase tracking-uppercase mb-2">{label}</p>
                                ...
                                <p className="text-xs text-muted-foreground/80 mt-1">{data?.date?.slice(0, 7) ?? 'Latest'}</p>
                            </div>
                        ))}
                    </div>
                    ...
                    <div className="p-5 rounded-2xl bg-card border border-border shadow-sm">
                        <p className="text-xs font-black text-amber-700 dark:text-amber-400 uppercase tracking-uppercase mb-2">📡 Analyst Insight</p>
                        <p className="text-xs text-muted-foreground leading-relaxed">
                            <strong className="text-foreground">PMI Divergence:</strong> When Caixin PMI significantly outperforms NBS PMI, it suggests private-sector SMEs are recovering faster than state-owned enterprises — a positive signal for urban employment and consumption. Watch for convergence as a regime confirmation trigger.
                        </p>
                    </div>
                </>
            )}
