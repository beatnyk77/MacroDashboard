...
export const AfricaMacroSnapshot: React.FC = () => {
    ...
    return (
        <div className="relative space-y-8">
            {/* Main Container */}
            <div className="p-8 md:p-12 rounded-[3rem] bg-card border border-border shadow-sm dark:shadow-2xl overflow-hidden">
                {/* Header Section */}
                <div className="relative z-10 flex flex-col lg:flex-row justify-between items-start gap-8 mb-16">
                    <div className="flex-1 space-y-4">
                        <div className="flex items-center gap-3">
                            ...
                            <span className="text-xs font-black uppercase tracking-uppercase text-muted-foreground">Africa Macro Pulse</span>
                        </div>
                        <h2 className="text-3xl md:text-5xl font-black tracking-heading text-foreground leading-none">
                            AFRICA MACRO <span className="text-transparent bg-clip-text bg-gradient-to-r from-blue-500 to-emerald-500 dark:from-blue-400 dark:to-emerald-400">PULSE</span>
                        </h2>
                        <div className="flex items-center gap-4 pt-2">
                            <div className="flex items-center gap-2 text-xs font-bold text-blue-500 dark:text-blue-400/80 uppercase tracking-widest">
                                <Calendar size={14} />
                    ...
                    <div className="w-full lg:max-w-md p-6 rounded-2xl bg-muted/30 dark:bg-white/[0.02] border border-border backdrop-blur-md">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-16">
                    {snapshot.metrics_summary?.map((metric) => (
                        <div key={metric.name} className="p-6 rounded-2xl bg-card border border-border group hover:bg-muted/40 transition-all shadow-sm">
                            <div className="text-[10px] font-black text-muted-foreground uppercase tracking-widest mb-2">{metric.name}</div>
                            <div className="flex items-end gap-2">
                                <div className="text-2xl font-black text-foreground tabular-nums">{metric.value}{metric.unit}</div>
                                <div className={cn(
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6 pt-12 border-t border-border">
                    {/* Positive Insights */}
                    <div className="p-8 rounded-[2rem] bg-emerald-500/[0.04] border border-emerald-500/15 space-y-6 group hover:bg-emerald-500/[0.08] transition-all">
                        <div className="flex items-center gap-3">
                        ...
                        <ul className="space-y-4">
                            {snapshot.insights_positive?.map((insight, i) => (
                                <li key={i} className="text-xs text-muted-foreground leading-relaxed font-medium flex gap-3">
                                    <div className="w-1 h-1 rounded-full bg-emerald-500/60 mt-1.5 flex-shrink-0" />
                                    {insight}
                                </li>
                            ))}
                        </ul>
                    ...
                    {/* Neutral Insights */}
                    <div className="p-8 rounded-[2rem] bg-amber-500/[0.04] border border-amber-500/15 space-y-6 group hover:bg-amber-500/[0.08] transition-all">
                        <div className="flex items-center gap-3">
                        ...
                        <ul className="space-y-4">
                            {snapshot.insights_neutral?.map((insight, i) => (
                                <li key={i} className="text-xs text-muted-foreground leading-relaxed font-medium flex gap-3">
                                    <div className="w-1 h-1 rounded-full bg-amber-500/60 mt-1.5 flex-shrink-0" />
                                    {insight}
                                </li>
                            ))}
                        </ul>
                    ...
                    {/* Negative Insights */}
                    <div className="p-8 rounded-[2rem] bg-rose-500/[0.04] border border-rose-500/15 space-y-6 group hover:bg-rose-500/[0.08] transition-all">
                        <div className="flex items-center gap-3">
                        ...
                        <ul className="space-y-4">
                            {snapshot.insights_negative?.map((insight, i) => (
                                <li key={i} className="text-xs text-muted-foreground leading-relaxed font-medium flex gap-3">
                                    <div className="w-1 h-1 rounded-full bg-rose-500/60 mt-1.5 flex-shrink-0" />
                                    {insight}
                                </li>
                            ))}
                        </ul>
