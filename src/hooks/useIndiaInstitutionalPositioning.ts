  ...
  return useQuery({
    ...
    queryFn: async () => {
      const [{ data: snapshots, error: snapshotError }, { data: sectors, error: sectorError }, { data: marketRows, error: marketError }] = await Promise.all([
        ...
        supabase.from('metric_observations').select('metric_id, as_of_date, value').in('metric_id', ['IN_FII_CASH_NET', 'IN_DII_CASH_NET', 'IN_NIFTY_RETURN', 'IN_USD_INR_RETURN', 'IN_INDIA_VIX']).order('as_of_date', { ascending: true }).limit(1500),
      ]);
      ...
      for (const row of marketRows ?? []) {
        ...
        if (row.metric_id === 'IN_NIFTY_RETURN') point.nifty = Number(row.value);
