    ...
    return (
        <div className="min-h-screen bg-[#020202] text-white pt-24 pb-20">
            <div className="max-w-7xl mx-auto px-6">
                <div className="mb-8">
                    <Link to="/us-equities" className="inline-flex items-center text-xs font-bold uppercase tracking-uppercase text-white/40 hover:text-white transition-colors mb-6">
                    ...
                    <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
                        <div>
                            ...
                            <p className="text-sm font-medium text-white/40 uppercase tracking-uppercase">{company.sector || 'N/A'} &bull; {company.industry || 'N/A'}</p>
                        </div>
                        <div className="flex flex-wrap gap-6 text-right items-end">
                            <div>
                                <p className="text-xs uppercase tracking-uppercase font-black text-white/30 mb-1">CIK</p>
                                <p className="text-xl font-black text-white/60">{company.cik}</p>
                            </div>
                            <div>
                                <p className="text-xs uppercase tracking-uppercase font-black text-white/30 mb-1">Fiscal Year End</p>
                                ...
                                <div className="flex bg-black/40 p-1 rounded-xl border border-white/5 overflow-x-auto">
                                    {['revenue', 'net_income', 'roe', 'debt_equity', 'operating_margin'].map(metric => (
                                        <button
                                            ...
                                            className={`px-3 py-1.5 rounded-lg text-xs font-bold uppercase tracking-uppercase transition-all whitespace-nowrap ${chartMetric === metric ? 'bg-white/10 text-white' : 'text-white/40 hover:text-white/60'}`}
                                        >
                                            {metric.replace('_', ' ')}
                                        </button>
                                    ))}
                            ...
                            <div className="space-y-3">
                                {company.us_filings?.length > 0 ? (
                                    company.us_filings.sort((a: any, b: any) => new Date(b.filing_date).getTime() - new Date(a.filing_date).getTime()).map((f: any) => (
                                        ...
                                        >
                                            <div className="flex justify-between items-start">
                                                <div>
                                                    <span className="px-2 py-0.5 rounded bg-blue-500/10 text-blue-400 text-xs font-black uppercase tracking-uppercase mr-3">
                                                        {f.form_type}
                                                    </span>
                                    ...
                                    <div className="py-12 text-center border border-dashed border-white/5 rounded-2xl">
                                        <p className="text-xs font-bold text-white/20 uppercase tracking-uppercase">No recent filings ingested.</p>
                                    </div>
                                )}
                            </div>
                        </div>
                        ...
                        <div className="p-6 rounded-3xl bg-white/[0.02] border border-white/5">
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                <div className="p-4 rounded-2xl bg-black/40 border border-white/5">
                                    <p className="text-xs uppercase tracking-uppercase font-black text-white/30 mb-1">P/E Ratio</p>
                                    <p className="text-xl font-black">{latestFund.pe_ratio?.toFixed(1) || '—'}</p>
                                </div>
                                <div className="p-4 rounded-2xl bg-black/40 border border-white/5">
                                    <p className="text-xs uppercase tracking-uppercase font-black text-white/30 mb-1">P/B Ratio</p>
                                    <p className="text-xl font-black">{latestFund.pb_ratio?.toFixed(1) || '—'}</p>
                                </div>
                                <div className="p-4 rounded-2xl bg-black/40 border border-white/5">
                                    <p className="text-xs uppercase tracking-uppercase font-black text-white/30 mb-1">EV/EBITDA</p>
                                    <p className="text-xl font-black">{latestFund.ev_ebitda?.toFixed(1) || '—'}</p>
                                </div>
                                <div className="p-4 rounded-2xl bg-black/40 border border-white/5">
                                    <p className="text-xs uppercase tracking-uppercase font-black text-white/30 mb-1">FCF Yield</p>
                        ...
                        <div className="p-6 rounded-3xl bg-white/[0.02] border border-white/5">
                            ...
                            {company.us_insider_trades?.length > 0 ? (
                                ...
                                <div className="py-8 text-center bg-black/20 rounded-2xl">
                                    <p className="text-xs font-bold text-white/20 uppercase tracking-uppercase">No recent transactions.</p>
                                </div>
                            )}
                        </div>
                    </div>
