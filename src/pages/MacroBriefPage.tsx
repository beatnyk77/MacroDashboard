...
const MacroBriefInner: React.FC = () => {
    ...
    return (
      <div className="py-16 text-center space-y-4">
        ...
        <p className="text-sm text-white/40 max-w-md mx-auto">
          Could not retrieve a macro brief for this date. Check back after 05:30 AM ET.
        </p>
        <Link
          to="/macro-brief"
  ...
  const seoDescription = activeBrief.content.regime_status
    ? activeBrief.content.regime_status.slice(0, 155)
    : 'Daily institutional macro intelligence brief from GraphiQuestor.';

  // Derived Section 3 Heading
  const focusAreaNames = selectedAreas
    .map((a) => FOCUS_AREA_LABELS[a]?.replace(/[\uD800-\uDBFF][\uDC00-\uDFFF]\s*/g, '')) // Remove emojis for header text clean look
    .join(' & ');
  ...
  return (
    <>
      <SEOManager
        title={`Morning Macro Brief — ${format(parsedDate, 'MMMM d, yyyy')} | GraphiQuestor`}
        description={seoDescription}
        ogType="article"
        publishedTime={activeBrief.generated_at}
        canonicalUrl={`https://graphiquestor.com/macro-brief/${activeBrief.brief_date}`}
        robots="index, follow"
      />
      ...
      <div className="space-y-8">
        ...
        {isFallback && (
          <div className="flex items-start gap-3 p-4 rounded-xl bg-amber-500/10 border border-amber-500/30 text-amber-400">
            ...
            <div className="text-xs font-mono uppercase tracking-wider leading-relaxed">
              Today&apos;s brief is being generated. Showing previous session&apos;s brief. Refresh after 5:30 AM ET.
            </div>
          </div>
        )}
          ...
          <div className="flex items-center gap-3">
            ...
            {activeBrief.regime_score !== null && (
              <span className="text-xs font-mono text-white/40">
                SCORE: {activeBrief.regime_score}
              </span>
            )}
          </div>
          <span className="text-xs font-mono text-white/30">
            Based on 45 macro signals
          </span>
        </div>
        ...
        <div className="space-y-3 bg-white/[0.02] border border-white/5 rounded-2xl p-6">
          ...
          <div className="text-[10px] font-mono text-white/20 uppercase tracking-widest">
            Brief tailors to your selection (max 3, sliding window)
          </div>
        </div>
